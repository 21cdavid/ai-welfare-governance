...
const filters = [
  { key: 'all', label: '전체' },
  { key: '사회복지정책', label: '사회복지정책' },
  { key: 'AI·데이터', label: 'AI·데이터' },
  { key: '법·행정', label: '법·행정' },
  { key: '현장·실무', label: '현장·실무' },
]
...
export default function ExpertsPage() {
  ...
  const filtered = experts.filter(e => {
    ...
    const matchSearch = search === '' ||
      ...
      (e.keywords || []).some((k: string) => k.includes(search))
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <input
          ...
          className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm mb-4 focus:outline-none focus:border-blue-400"
          value={search}
        ...
        <div className="flex gap-2 flex-wrap mb-6">
          {filters.map(f => (
            <button
              ...
              className={`text-sm px-4 py-1.5 rounded-full border transition-colors ${filter === f.key ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-gray-600 border-gray-200 hover:bg-blue-50'}`}
            >
              {f.label}
            </button>
          ))}
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {filtered.map((e, i) => (
              <div key={e.id} className="bg-white rounded-2xl border p-5 text-center hover:border-blue-200 transition-all">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-medium mx-auto mb-3 ${colors[i % colors.length]}`}>
                  {getInitial(e.name)}
                </div>
                <p className="text-sm font-medium">{e.name}</p>
                <p className="text-xs text-gray-500 mb-1">{e.position}</p>
                <p className="text-xs text-gray-400 mb-3">{e.organization}</p>
